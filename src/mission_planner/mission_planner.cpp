...
#include "mission_planner_core.hpp"  // ⬅ 추가
...

{
  ...
  planner_->initialize(this);
    ...
    // 🔽 Core 초기화: map_frame / planner / transform_pose 콜백 전달
    core_ = std::make_unique<MissionPlannerCore>(
    map_frame_, planner_,
    [this](const Pose & pose, const Header & header) {
        return this->transform_pose(pose, header);
    });
...
LaneletRoute MissionPlanner::create_route(const SetLaneletRoute::Request & req)
{
  ...
  // Core 에게 route 생성을 위임한다.

   return core_->makeRouteFromSegments(
    header, segments, odometry_->pose.pose, goal_pose, uuid, allow_goal_modification);
...
LaneletRoute MissionPlanner::create_route(const SetWaypointRoute::Request & req)
{
  ...
  const Pose & start_pose = odometry_->pose.pose;

  return core_->makeRouteFromWaypoints(
...
{
    const LaneletRoute * current_route_ptr =
    current_route_ ? &(*current_route_) : nullptr;

  return core_->makeRouteForModifiedGoal(
    msg, current_route_ptr, odometry_->pose.pose);
